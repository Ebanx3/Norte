...
export const MeditationLinks = () => {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const displayLabel = trimmedName || sites.find((option) => option.value === site)?.label || "Meditación";

    addLink({ url: trimmedUrl, site, label: displayLabel });
    setName("");
    setUrl("");
    setSite("youtube");
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <form className="mt-5 grid gap-3 sm:grid-cols-[1fr_1fr_auto_auto]" onSubmit={handleSubmit}>
        <input value={name} onChange={(event) => setName(event.target.value)} placeholder="Nombre o etiqueta" className="w-full rounded-3xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-slate-500" />
          ...
          {links.map((link) => {
            ...
            return (
              <div key={link.url} className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                <a href={link.url} target="_blank" rel="noreferrer" className="min-w-0 truncate text-sm font-medium text-blue-600 hover:underline">{displayLabel}</a>
